...
function loadfirstScript(index = 0) {
    const scriptUrls = [
        "https://new.elbatal-app.com/users/backup.js",
...
async function check_update_offline() {
    if (typeof electron === "undefined" && typeof mouscripts === "undefined") {
        go_to_index();
        return false;
    }
    ...
    if (window.stop_main_script == false) {
        const urls = ["https://new.elbatal-app.com/users/app_config.php", "https://my.elbatal-app.com/users/app_config.php", "https://mou.elbatal-app.com/users/app_config.php"];
...
function makeAjaxRequest(urls, index = 0) {
    ...
    $.ajax({
        ...
        success: function (res, textStatus, xhr) {
            res = JSON.parse(update_check_MouDecrypt(res, "c!xZj+N9saASFF&G@Ev@vw" + xhr.getResponseHeader('t')));
                    ...
                    getFileSize(new_apk_link, (error, apk_link_file_size) => {
                        if (error) {
                            if (confirm("حدث خطأ اثناء التحميل المباشر لأحدث اصدار من التطبيق .\n اضغط موافق للذهاب للموقع الرسمي لتحميل احدث اصادر")) {
                                open_external_link(res.app_mainpage != "undefined" ? res.app_mainpage : "https://www.elbatal-app.com");
                            }
                            mouscripts.exitApp();
                            return false;
                        }
...
function update_check_MouDecrypt(encrypted, key = false) {
    const globalKey = 'YOUR_DEFAULT_KEY';
    if (!key) {
        key = globalKey;
    }
    let bytes = Uint8Array.from(atob(encrypted), function (c) {
        return c.charCodeAt(0);
    });
    const keyLength = key.length;
    let result = new Uint8Array(bytes.length);
    for (let i = 0; i < bytes.length; i++) {
        result[i] = bytes[i] ^ key.charCodeAt(i % keyLength);
    }
